...
interface RunDetails {
  ...
  metrics: {
    ...
    avg_step_confidence: number;
    steps_count: number;
    low_confidence_steps: number;
    high_latency_steps: number;
    can_replay: boolean;
  };
}
...
const RunDetail = () => {
  ...
  const fetchDetails = async () => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/run-details?run_id=${runId}`,
  ...
  if (!data) {
    return <div className="min-h-screen bg-background flex items-center justify-center text-muted-foreground">Run not found</div>;
  }

  const { run, steps, metrics } = data;
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-5xl mx-auto p-6 space-y-6">
          ...
          <CardContent className="p-6">
            <div className="flex items-start justify-between">
              <div>
                ...
                <h1 className="text-xl font-semibold mb-2">{run.input_query}</h1>
                <p className="text-sm text-muted-foreground">
                  Started {format(new Date(run.started_at), "MMM d, yyyy HH:mm:ss")}
                </p>
              </div>
              <div className="text-right">
                <div className={`text-3xl font-mono font-bold ${getConfidenceColor(run.confidence_score)}`}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <MiniMetric icon={<Clock />} label="Total Latency" value={`${metrics.total_latency_ms}ms`} />
          <MiniMetric icon={<Zap />} label="Tokens Used" value={run.token_usage} />
          <MiniMetric icon={<Target />} label="Steps" value={metrics.steps_count} />
          <MiniMetric 
            icon={<AlertTriangle />} 
            label="Warnings" 
            value={metrics.low_confidence_steps + metrics.high_latency_steps}
            color={metrics.low_confidence_steps > 0 ? "text-warning" : undefined}
          />
        </div>
          ...
          <CardContent className="space-y-2">
            {steps.map((step, index) => (
              <div key={step.id} className="relative">
                ...
                {expandedSteps.has(index) && (
                  <div className="ml-13 pl-8 py-4 space-y-4 animate-fade-in">
                    <div>
                      <h4 className="text-xs font-medium text-muted-foreground uppercase mb-2">Input</h4>
                      <pre className="p-3 rounded bg-background text-sm overflow-auto font-mono">
